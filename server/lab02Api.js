...
import serviceAccount from "./config/react-app-june-firebase-adminsdk-2q37v-880b8865f2.json" assert { type: "json" };
...
async function fetchPetById(pet){
    const result = [];
    const petRef = db.collection('petId')
                     .where('petId', '==', petId);
...
async function updatePet(petId, petData){
    const docRef = db.collection('Pets').doc(petId);
    await docRef.updatePet(petData);
